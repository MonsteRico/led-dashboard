...
LedMatrixAddon::LedMatrixAddon(const Napi::CallbackInfo &info)
	: Napi::ObjectWrap<LedMatrixAddon>(info), after_sync_cb_(Napi::FunctionReference()), fg_color_(Color(0, 0, 0)), bg_color_(Color(0, 0, 0)), font_(nullptr), font_name_(""), t_start_(get_now_ms()), t_sync_ms_(0), t_dsync_ms_(0)
{
	auto env = info.Env();
...
Napi::Value LedMatrixAddon::draw_buffer(const Napi::CallbackInfo &info)
{
	...
	for (unsigned int y = 0; y < h; y++)
	{
		...
		for (unsigned int x = 0; x < w; x++)
		{
			if (x > w)
				break;
			if (x + xO > this->matrix_->width() || x + xO < 0 || y + yO > this->matrix_->height() || y + yO < 0)
...
Napi::Object LedMatrixAddon::matrix_options_to_obj(const Napi::Env &env, const RGBMatrix::Options &options)
{
	...
	std::string pixel_mapper_config = options.pixel_mapper_config == NULL ? "" : std::string(options.pixel_mapper_config);
