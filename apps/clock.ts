...
export default class Clock extends App {
    ...
    public update() {
        ...
        this.matrix.drawImage(images["flowerBG.png"], 5, 0);
        this.matrix.fgColor(new Color("#ffffff"));
        this.matrix.font(fonts["spleen-8x16"]);
        this.matrix.drawText(this.time.toLocaleString(DateTime.TIME_SIMPLE), 0, 8, { leftShadow: true });
        this.matrix.font(fonts["6x9"]);
        this.matrix.fgColor(new Color("#ffffff"));
        this.matrix.drawText(this.time.toFormat("EEE LLL d"), 2, 21, { leftShadow: true });
    }
}
